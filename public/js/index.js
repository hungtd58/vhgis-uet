...
Vue.component('vhgis-map', {
    ...
    methods: {
        updateMap: function updateMap(year, month, day, size) {
            console.log("Update!!!!!!!!!");
            console.log(year + " " + month + " " + day + " " + size);
            var vm = this;
            if (vm.map == null) return;
            vm.map.removeMarkers();
            $.get(vm.prefix + "/gethistoryname?year=" + year, function (data, status) {
            ...
            $.get(vm.prefix + "/geteventlist?year=" + year + "&month=" + month + "&day=" + day + "&size=" + size, function (data, status) {
                if (data.result.length > 0) {
                    events = data.result;
